...
class UiLeds_duoled(DoorlockdBaseClass):
	...
	def hw_init(self):
		self.logger.info('initializing {}.'.format(self.config_name))
		self.led = DuoLed(self.config.get('led_red', "P8_13"), self.config.get('led_green', "P8_19"),'duoled')
	...
	def selftest(self):
		for ln in ['r','g','y']:
			...
			print("LED selftest: {}, {}, {}".format(ln, led.config_name, led.gpio_pin not None ? led.gpio_pin : '_no pin_'))
