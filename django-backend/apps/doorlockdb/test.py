...
class ViewTests(TestCase):
    def setUp(self):
        ...
        self.m["key_enabled_person_enabled"] = factory.RandomKey.create(
            is_enabled=True, owner=self.m["person_enabled"]
        )
        self.m["key_disabled_person_enabled"] = factory.RandomKey.create(
            is_enabled=False, owner=self.m["person_enabled"]
        )
        self.m["key_enabled_person_disabled"] = factory.RandomKey.create(
            is_enabled=True, owner=self.m["person_disabled"]
        )
        self.m["key_disabled_person_disabled"] = factory.RandomKey.create(
            is_enabled=False, owner=self.m["person_disabled"]
        )
        ...
        self.m["person_disabled_noaccess"] = factory.RandomPerson.create(
            is_enabled=False
        )
        # Key Enabled + Disabled for both persons:
        self.m["key_enabled_person_enabled_noaccess"] = factory.RandomKey.create(
            is_enabled=True, owner=self.m["person_enabled_noaccess"]
        )
        self.m["key_disabled_person_enabled_noaccess"] = factory.RandomKey.create(
            is_enabled=False, owner=self.m["person_enabled_noaccess"]
        )
        self.m["key_enabled_person_disabled_noaccess"] = factory.RandomKey.create(
            is_enabled=True, owner=self.m["person_disabled_noaccess"]
        )
        self.m["key_disabled_person_disabled_noaccess"] = factory.RandomKey.create(
            is_enabled=False, owner=self.m["person_disabled_noaccess"]
        )
    ...
    def test_lock_sync_ok(self):
        ...
        self.assertIn(
            self.m["key_enabled_person_enabled"].hwid, response.json()["keys"]
        )
        # should not show any of the other keys:
        self.assertNotIn(
            self.m["key_disabled_person_enabled"].hwid, response.json()["keys"]
        )
        self.assertNotIn(
            self.m["key_disabled_person_disabled"].hwid, response.json()["keys"]
        )
        self.assertNotIn(
            self.m["key_enabled_person_enabled_noaccess"].hwid, response.json()["keys"]
        )
        self.assertNotIn(
            self.m["key_disabled_person_enabled_noaccess"].hwid, response.json()["keys"]
        )
        self.assertNotIn(
            self.m["key_disabled_person_disabled_noaccess"].hwid,
            response.json()["keys"],
        )
        self.assertNotIn(
            self.m["key_enabled_person_disabled_noaccess"].hwid, response.json()["keys"]
        )
    ...
    def test_lock_disabled_sync_error(self):
        ...
        self.assertNotContains(
            response, self.m["key_enabled_person_enabled_noaccess"].hwid
        )
        self.assertNotContains(
            response, self.m["key_disabled_person_enabled_noaccess"].hwid
        )
        self.assertNotContains(
            response, self.m["key_disabled_person_disabled_noaccess"].hwid
        )
        self.assertNotContains(
            response, self.m["key_enabled_person_disabled_noaccess"].hwid
        )
