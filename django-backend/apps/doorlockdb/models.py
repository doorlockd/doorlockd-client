...
class Person(models.Model):
    ...
    personsgroup = models.ManyToManyField('PersonGroup', blank=True, related_name='persons')
    ...
    def last_seen_start(self):
        ...
        for k in self.key_set.all():
            if last_seen_start is None or k.last_seen_start is not None and k.last_seen_start > last_seen_start:
    ...
    def last_seen_end(self):
        last_seen_end = None
        for k in self.key_set.all():
            if last_seen_end is None or k.last_seen_end is not None and k.last_seen_end > last_seen_end:
...
class Key(models.Model):
    ...
    description = models.CharField(max_length=200, help_text='Enter a meaning full description. example="NS OV chipkaart t.h.t. 02-2027"')
    is_enabled = models.BooleanField(default=True)
...
class LogKeyLastSeen(models.Model):
    ...
    def addLastSeen(hwid, lock, last_seen_start, last_seen_end, count=1):
        ...
        #
        SyncLockKeys.objects.filter(lock=lock).update(last_log_keys = datetime.datetime.now())
        ...
        try:
            ...
            logger.critical(f"LastSeen update failed: Key ({hwid}) doesn't exist! ({hwid}, {lock}, {last_seen_start}, {last_seen_end}, {count})")
            return 

        # update  last_seen_end if newer, last_seen_start if newer, update count, update lock
        k_log, bool_value_is_new = LogKeyLastSeen.objects.get_or_create(key=k, defaults={'last_seen_start':last_seen_start, 'last_seen_end':last_seen_end, 'counter':count, 'lock':lock})
...
class Lock(models.Model):
    name = models.CharField(max_length=32, unique=True)
    description = models.CharField(max_length=200, blank=True)
    is_enabled = models.BooleanField(default=True, help_text="Disable access for all keys on this lock. This lock is still able to synchronize with it's SSL certificate.")
    # last_synced = models.DateTimeField(null=True,default=None)
    certificate = models.TextField(max_length=2000, unique=True, null=True, default=None, blank=True, help_text=f"Paste client certitificate here (including '-----BEGIN CERTIFICATE-----' and '-----END CERTIFICATE-----').<br>\n<br>\nOn the client configure this server server Fingerprint. (hint: restart django after server ssl certificate is changed). <br>\n<code>server_ssl_fingerprint='{getServerSSLFingerprint()}'</code>")
    ...
    def custom_all_persons(self):
        ...
        return Person.objects.filter(is_enabled=True, personsgroup__is_enabled=True, personsgroup__access_groups__locks=self).distinct()
    ...
    def custom_all_keys(self):
        ...
        return Key.objects.filter(is_enabled=True, owner__is_enabled=True, owner__personsgroup__is_enabled=True, owner__personsgroup__access_groups__locks=self).distinct()
    ...
    def clean(self):
        ...
        if self.certificate is not None:
            ...
            if self.certificate == '' or self.certificate.splitlines()[0] != '-----BEGIN CERTIFICATE-----' or self.certificate.splitlines()[-1] != '-----END CERTIFICATE-----':
                raise ValidationError({'certificate': "Must begin with '-----BEGIN CERTIFICATE-----' and end with '-----END CERTIFICATE-----'"})
...
class LogUnknownKey(models.Model):
    ...
    def register(hwid, lock, last_seen=None, count=1):
        ...
        # set lock last_seen stats:
        #
        SyncLockKeys.objects.filter(lock=lock).update(last_log_unknownkeys = datetime.datetime.now())
        ...
        # do + count
        LogUnknownKey.objects.filter(hwid=hwid).update(lock=lock, last_seen=last_seen, counter=F('counter') + count)
...
class Helpers:
    ...
    def create_accesslist_for_lock__dict(cls, lock:Lock):
        ...
        for ag in AccessGroup.objects.filter(locks = lock):
            ...
            #
            keys = []
            for pg in PersonGroup.objects.filter(access_groups = ag, is_enabled=True).all():
    ...
    def related_objects_for_lock(cls, lock:Lock):
        ...
        r['PersonGroup'] = PersonGroup.objects.filter(access_groups__in= r['AccessGroup'] )
    ...
    def get_hash_for_dict(cls, data):
        ...
        return hashlib.new('SHA256', json.dumps(data, sort_keys=True).encode('utf-8')).hexdigest()
...
def post_update_check_sync(sender, **kwargs):
    ...
    elif type(kwargs.get('instance')) in  [ Key, PersonGroup,AccessGroup,AccessRule, AccessRuleset, Person ]:
