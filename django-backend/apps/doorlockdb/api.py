...
def on_client_ssl_cert_error(request, exc):
    return api.create_response(request, {"error": f"Client SSL Certificate error :'{exc}'"}, status=401)
...
@api.post("/lock/sync.keys", response={200: SyncKeysOutputSchema, 401: ErrorOutputSchema})
...
# Sync unknownkeys
#
@api.post("/lock/log.unknownkeys", response={200: LogUnknownKeysOutputSchema, 401: ErrorOutputSchema})
...
@api.post("/lock/log.keys_last_seen", response={200: LogKeysLastSeenOutputSchema, 401: ErrorOutputSchema})
def api_lock_log_keys_last_seen(request, input_data: LogKeysLastSeenInputSchema):
    ...
    for k in input_data.keys_last_seen:
        try:
            LogKeyLastSeen.addLastSeen(k.key, l, k.timestamp_begin, k.timestamp_end, k.count)
            saved.append(k)
        except Exception as e:
            raise Exception(e)
