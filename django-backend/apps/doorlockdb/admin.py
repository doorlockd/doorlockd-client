...
class PersonAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        return queryset.annotate(key_count=Count('key', distinct=True)).annotate(group_count=Count('personsgroup',distinct=True))
...
class KeyAdmin(admin.ModelAdmin):
    ...
    def render_change_form(self, request, context, *args, **kwargs):
        ...
        if request.GET.get('unknownkey'):
            context['adminform'].form.fields['hwid'].widget.attrs['value'] = request.GET.get('unknownkey')
        ...
        # browsing to "new" Key
            ...
            for k in LogUnknownKey.objects.order_by('-last_seen'):
                unknownkeys.append((k.hwid, f"last_seen={k.last_seen.strftime('%H:%M %d-%m-%Y')}, lock={k.lock}, counter={k.counter} , hwid={k.hwid}"))

            context['adminform'].form.fields['hwid'] = forms.ChoiceField(help_text='A valid hwid, please.', choices=unknownkeys, initial='0', required = True,)
...
class LogUnknownKeyAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ("hwid", "last_seen", "lock", "counter",)
    ordering = ("-last_seen",)
    #
...
class LogKeyLastSeenAdmin(admin.ModelAdmin):
    list_display = ("key","lock","owner", "counter",  "last_seen_start", "last_seen_end")
...
class AccessRuleInline(admin.StackedInline):
    ...
    fieldsets = (('edit rule', {
            'classes': ('collapse',),
            'fields': ('after', 'before', 'weekdays_monday', 'weekdays_tuesday', 'weekdays_wednesday', 'weekdays_thursday', 'weekdays_friday', 'weekdays_saturday', 'weekdays_sunday', 'time_start', 'time_end')
        }),)
...
class SyncLockKeysAdmin(admin.ModelAdmin):
    list_display = ("lock","config_time","last_seen", "synchronized","last_sync_keys","last_log_unknownkeys","last_log_keys" )
    readonly_fields = ("lock","config_time","last_seen", "synchronized","last_sync_keys","last_log_unknownkeys","last_log_keys","keys_json")
