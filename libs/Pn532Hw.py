...
class Led(DoorlockdBaseClass, LedMethods):
    ...
    def __init__(
        ...
        if pn532_gpio is None:
            raise ValueError(
                "argument pn532_gpio must be set to an instance of the pn532gpio class"
            )
    ...
    def hw_init(self):
        self.pn532_gpio.cfg_gpio_set(
            self.gpio_pin, 0x3, self.no_cache_cfg
        )  # 0x3 : 'Push/pull output'
        self.logger.info(
            "initializing {} on gpio pin PN532:{:s}.".format(
                self.config_name, str(self.gpio_pin)
            )
        )
...
class UiLeds_4leds_Pn532(UiLeds_4leds):
    ...
    def hw_init(self):
        ...
        self.l1 = Led(
            self.config.get("led1", "p30"), "led1", pn532_gpio=self.pn532_gpio
        )
        self.l2 = Led(
            self.config.get("led2", "p31"), "led2", pn532_gpio=self.pn532_gpio
        )
        self.l3 = Led(
            self.config.get("led3", "p32"), "led3", pn532_gpio=self.pn532_gpio
        )
        self.l4 = Led(
            self.config.get("led4", "P33"), "led4", pn532_gpio=self.pn532_gpio
        )
...
class Pn532Button(DoorlockdBaseClass):
    ...
    def hw_init(self):
        self.pn532_gpio.cfg_gpio_set(
            self.gpio_pin, 0x2, self.no_cache_cfg
        )  # 0x2 : 'input'
        self.logger.info(
            "initializing {} on gpio pin PN532:{:s}.".format(
                self.config_name, str(self.gpio_pin)
    ...
    def trigger(self):
        ...
        dc.e.raise_event(
            self.trigger_action
        )  # raise configured trigger_action for this Button
        self.counter = self.counter + 1

    def hw_exit(self):
        """exit/de-initialize gpio port."""
        self.logger.info(
            "exitting {} on gpio pin {:s}.".format(self.config_name, str(self.gpio_pin))
        )
