...
class UILed4(module.BaseModule):
	...
	def enable(self):
		...
		if self.mod_rfid is not None:
			assert hasattr(self.mod_rfid, 'rfid'), "[ui4led] rfid: configured module is not compatible. (.rfid)"
			assert hasattr(self.mod_rfid.rfid, 'state_reader_ready') and isinstance(self.mod_rfid.rfid.state_reader_ready, State), "[ui4led] rfid: configured module is not compatible. (state)"
			assert hasattr(self.mod_rfid, 'events') and isinstance(self.mod_rfid.events, Events), "[ui4led] rfid: configured module is not compatible. (events)"
			...
			self.s.append(self.mod_rfid.rfid.state_reader_ready.subscribe(lambda v: self.io_led1.output(v)))

			self.s.append(events.subscribe('rfid_comm_pulse', lambda data: self.led2.blink()))
			self.s.append(events.subscribe('rfid_comm_ready', lambda data: self.led2.blink()))
			self.s.append(events.subscribe('rfid_access_denied', lambda data: self.led3.on()))
			self.s.append(events.subscribe('rfid_access_denied_fin', lambda data: self.led3.off()))
				
		# blink_on_buttons
		for btn in self.blink_on_buttons:
			mod =  dc.module.get(btn, None) 
			if mod is not None:
				assert hasattr(mod, 'events') and isinstance(mod.events, Events), "[ui4led] {}: configured module is not compatible. (events)".format(btn)
				logger.info('UILed4 blink on button {} {}'.format(btn, mod))
				self.s.append(mod.events.subscribe('pressed', lambda data: self.led2.blink() ))
