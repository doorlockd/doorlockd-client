...
class HbwVoordeurLogic:
	...
	def __init__(self, config={}):
		...
		self.event_cancel_open_solenoid  = config.get('event_cancel_open_solenoid', 'cancel_open_solenoid')

		# state values:
		self.state_dagslotschootdetectie = State()
		self.state_nachtslotschootdetectie = State()
		
		self.state_dagslotschootdetectie.subscribe(lambda v: logger.info(f"state_dagslotschootdetectie = {v}"))
		self.state_nachtslotschootdetectie.subscribe(lambda v: logger.info(f"state_nachtslotschootdetectie = {v}"))
		...
		self.event_doorhandle_opened_name = config.get('event_doorhandle_opened', 'doorhandle_opened')
	...
	def enable(self):
		...
		if self.enable_cancelopensolenoid_on_dooropened:
			...
			self.subscription_dooropened_cancelsolenoid = dc.e.subscribe(self.event_door_opened_name, lambda d: dc.e.raise_event(self.event_cancel_open_solenoid))
