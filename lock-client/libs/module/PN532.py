...
class PN532(module.BaseModule):
	...
	def __init__(self, config={}):
		...
		if config.get('rfid_enabled', False):
			self.rfid = RfidReader(self.clf, self.events, config.get('rfid_event', 'rfid_event'))
		...
		for k in config['io_export'].keys():
			...
			dc.io_port[k] = io_pn532.Port(port, limit_direction=limit_direction, active_low=active_low)
...
class RfidReader:	
	def __init__(self, clf, event_bus, event):
		...
		self.state_reader_ready.subscribe(lambda data: logger.debug("PN532.rfid.state_reader_ready: %s|%s", str(data), str(self.state_reader_ready.value)))
	...
	def callback_tag_detected(self, target):
		...
		if dc.rfid_auth.has_access(hwid_str):
			...
			self.event_bus.raise_event('rfid_access_allowed') # raise when rfid is access_allowed
			...
			self.event_bus.raise_event('rfid_access_denied') # raise when rfid is access_denied, will folow by ..._fin in x seconds
			time.sleep(3)
			self.event_bus.raise_event('rfid_access_denied_fin') # raise when rfid is access_denied after sleeping x seconds.
	...
	def start_thread(self):	
		if not (self.thread and self.thread.is_alive()):
			...
			logger.info('notice: {:s}: start_thread, thread is already running '.format('PN532 RFiD Reader'))
